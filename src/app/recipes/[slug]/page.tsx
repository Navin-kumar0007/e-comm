                      ...
                      {recipe.products.map(p => (
                        <Link href={`/product/${p.slug}`} key={p.id} className="flex items-center gap-3 group">
                          <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 border">
                            <Image width={800} height={800} unoptimized={false} alt={p.name} src={typeof p.images === "string" && p.images.startsWith("[") ? JSON.parse(p.images)[0] : (Array.isArray(p.images) ? p.images[0] : p.images)} className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
                          </div>
